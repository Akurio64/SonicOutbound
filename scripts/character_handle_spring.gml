/// character_handle_spring()
 ...
 // Trigger spring.
    ...
    // Spring handle:
    var _seesaw;
        _seesaw = instance_nearest(x, y, obj_seesaw);
    ...
    if _seesaw.bounce=false
    {
    if(character_collision_check(COL_BOTTOM_OBJECT, MASK_BIG, x, y, angle, _seesaw) && y_speed >= 0)
             {
                if _seesaw.state=0{_seesaw.alarm[1]=1}
                
        
             } }
             else{
       if(_seesaw.spring_direction == SP_UP)
       { 
          if(angle != 90 && angle != 270)
          {
             if(character_collision_check(COL_BOTTOM_OBJECT, MASK_BIG, x, y, angle, _seesaw) && y_speed >= 0)
          ...
          else
          {
             if(angle == 90)
             {
                if(character_collision_check(COL_LEFT_OBJECT, MASK_BIG, x, y, angle, _seesaw) && sign(x_speed) >= 0.0)
                {
                   // Bounce the character.
                      x_speed             = _seesaw.spring_strength;
                      animation_direction = 1;
             ...
             if(angle == 270)
             {
                if(character_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, angle, _seesaw) && sign(x_speed) >= 0.0)
                {
                   // Bounce the character.
                      x_speed             = -_seesaw.spring_strength;
                      animation_direction = -1;
