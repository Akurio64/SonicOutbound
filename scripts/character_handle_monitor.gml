/// character_handle_monitor()
 ...
 // Destroy from the top.
    if(_monitor != noone)
    {
       ...
       {
          if(state = CS_JUMP && !ground || state = CS_ROLL && !ground || animation_index = "ROLL" && !ground || shield_state == 1 || state = CS_TORNADO && !ground || state = CS_MEGA && !ground)
          {
             // Destroy monitor:
                _monitor.parent = id;             
                with(_monitor)
 ...
 // Destroy from the side.
    if(_monitor != noone)
    {
       ...
       {
          if(state == CS_ROLL || shield_state == 1 || state == CS_GLIDE || state == CS_SLIDE || state == CS_TORNADO || state == CS_MEGA)
          {
             // Destroy monitor:
                _monitor.parent = id;                        
                with(_monitor)
